<script>
    ...
    function notificationClick(e, notification) {
        console.log("Клик по карточке:", notification, "Текущий список:", $notificationsList);
        
        const targetId = notification._localId || notification.timestamp;
        
        $notificationsList = $notificationsList.filter(n => {
            const nId = n._localId || n.timestamp;
            return nId !== targetId;
        });
        
        window.notificationsApi.save($notificationsList);
...
>
    <div class="notifications-modal-titlebar">
        ...
        <div class="modal-buttons flex-row">
            <button
                onclick={() => {
                    console.log('Клик по корзине сработал!', $notificationsList);
                    $notificationsList = [];
                    notificationCount.set(0);
                    window.notificationsApi.save([]);
                    anixApi.notification.removeAllNotifications();
                }}
            >
                <Icon
                    src={DeleteIcon}
