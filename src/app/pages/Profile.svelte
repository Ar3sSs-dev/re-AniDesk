<script>
    ...
    profile.then((data) => {
        if (!data || !data.profileInfo || !data.profileInfo.profile) return;
...
{#await profile}
    <Preloader />
{:then p}
    {#if p && p.profileInfo && p.profileInfo.profile}
        {#key modalSubTitle}
    ...
    {:else}
        <div class="notify-center">
            <span class="third-text">Профиль не найден или недоступен</span>
        </div>
    {/if}
{:catch error}
    <div class="notify-center">
        <span class="third-text">Ошибка при загрузке профиля</span>
    </div>
{/await}
